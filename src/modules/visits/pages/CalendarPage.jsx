...
// ─── Tooltip flotante ─────────────────────────────────────────────────────────
const EventTooltip = ({ event, position, agents, getAgentColor, getAgentName, getClientName, getPropertyTitle, onClose, onMouseEnterTooltip, onEdit, onComplete, onWhatsApp }) => {
  const r = event?.resource || {};
...
const CalendarPage = () => {
  ...
  const handleMouseLeaveEvent = useCallback(() => {
    clearTimeout(tooltipTimer.current);
    tooltipTimer.current = setTimeout(() => setTooltip({ event: null, position: null }), 500);
  }, []);
    ...
    <div className="cal-page">
      ...
      <AnimatePresence>
        {tooltip.event && tooltip.position && (
          <EventTooltip
            ...
            onMouseEnterTooltip={() => clearTimeout(tooltipTimer.current)}
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          .cal-kpi-row { gap: 0.5rem; }
          .cal-kpi { flex: 1 1 45%; padding: 0.625rem 0.75rem; }
          .cal-kpi-num { font-size: 1.125rem; }
          .cal-kpi-icon { font-size: 1.125rem; }
          .cal-filters-card { padding: 0.75rem; }
          .cal-wrapper .rbc-calendar { font-size: 11px; }
          .cal-wrapper .rbc-header { font-size: 10px; padding: 0.375rem 0; }
          .rbc-custom-toolbar { flex-direction: column; gap: 0.5rem; padding: 0.5rem; }
          .rbc-toolbar-left { width: 100%; justify-content: space-between; }
          .rbc-toolbar-views { width: 100%; justify-content: center; }
          .rbc-current-label { min-width: auto; font-size: 12px; text-align: center; }
          .cal-event-chip { font-size: 10px; padding: 1px 3px; }
          .cal-event-chip-dot { width: 5px; height: 5px; }
          .cal-tooltip { width: 260px; }
          .cal-modal { max-width: 95vw; }
          .cal-btn-primary { padding: 0.5rem 0.75rem; font-size: 12px; }
        }
      `}</style>
