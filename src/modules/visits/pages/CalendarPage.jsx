...
export default function CalendarPage() {
  ...
  // ── Vista semanal: 7 días desde lunes de la semana actual ────
  ...
  const DayCell = ({ date, mini = false }) => {
    if (!date) return <div className="h-24 sm:h-28" />;
    ...
    return (
      ...
      >
        ...
        <div className="mt-1 space-y-0.5 overflow-hidden">
          ...
          {dayEvs.length > 3 && (
            <p className="text-[10px] text-slate-500 px-1">+{dayEvs.length - 3} más</p>
          )}
        </div>
      </div>
    );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {['month','week'].map((v) => (
              <button
                ...
                className={`px-4 py-1.5 text-xs font-semibold transition-colors ${
                  view === v ? 'bg-primary text-slate-950' : 'bg-slate-900 text-slate-400 hover:text-white'
                }`}
              >
                {v === 'month' ? 'Mes' : 'Semana'}
              </button>
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm font-bold min-w-[160px] text-center" style={{ color: 'var(--color-text)' }}>
              {title}
            </span>
            <button
              onClick={() => navigate(1)}
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-slate-400 hover:text-white text-xs transition-colors"
            >
              Hoy
            </button>
          </div>
      ...
      {loading ? (
        ...
        <>
          {/* Vista mensual */}
          {view === 'month' && (
            <div className="rounded-2xl border border-slate-800 overflow-hidden" style={{ background: 'var(--color-surface)' }}>
              {/* Cabecera días */}
          ...
          {view === 'week' && (
            <div className="rounded-2xl border border-slate-800 overflow-hidden" style={{ background: 'var(--color-surface)' }}>
              <div className="grid grid-cols-7 border-b border-slate-800">
                {weekDays.map((d, i) => (
                  <div key={i} className={`text-center py-2 text-xs font-bold ${
                    toKey(d) === today ? 'text-primary' : 'text-slate-400'
                  }`}>
      ...
      <AnimatePresence>
        {selected && (
          <>
            ...
            >
              <div
                className="w-full max-w-sm rounded-2xl border border-slate-700 p-6 space-y-4 pointer-events-auto"
...
function InfoRow({ icon, label, value, accent }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-xs font-semibold ${accent ? 'text-yellow-400' : ''}`}
