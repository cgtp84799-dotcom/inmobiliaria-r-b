...
//
// PANEL AGENTE (admin / member):
//   Tabs: Perfil — Métricas — Actividad — Contratos
//   Sin cambios respecto a la versión original.
//
// PANEL CLIENTE (viewer / portal):
//   - KPIs: favoritos, visitas, contratos, valor total
//   - Info de contacto + preferencias del cliente
//   - Lista de visitas recientes (con estado cancelledByClient)
//   - Lista de contratos recientes
//   - Botón "Agendar visita" → navega a /agendar-visita pre-llenado
//   - Botón "Notificar" → formulario inline para enviar notificación al cliente
//   - Toggle activar/desactivar acceso al portal
...
import {
  FaTimes, FaEnvelope, FaPhone, FaCalendarAlt,
  FaEdit, FaToggleOn, FaToggleOff, FaKey,
  FaHome, FaCalendarCheck, FaFileContract,
  FaChartBar, FaHistory, FaCheckCircle,
  FaClock, FaTimesCircle as FaX, FaCircle,
  FaMapMarkerAlt, FaMoneyBillWave, FaBuilding,
  FaBell, FaPaperPlane, FaSpinner,
} from 'react-icons/fa';
import {
  USER_ROLES, USER_ROLE_LABELS, USER_ROLE_BADGE_CLASSES, USER_ROLE_DESCRIPTIONS,
} from '../types/user.types';
import {
  ROLE_ICONS, ROLE_ICON_CLASSES, STATUS_STYLES, STATUS_LABELS,
...
const NOTIF_OPTIONS = [
  { value: NOTIF_TYPES.MANUAL,       label: '📢 Mensaje general'   },
  { value: NOTIF_TYPES.NEW_PROPERTY, label: '🏠 Nueva propiedad'   },
  { value: NOTIF_TYPES.WELCOME,      label: '👋 Bienvenida'        },
];

function SendNotifForm({ email, onDone }) {
  const [form,    setForm]    = useState({ title: '', message: '', type: NOTIF_TYPES.MANUAL });
  ...
  async function handleSend() {
    ...
    try {
      ...
      setForm({ title: '', message: '', type: NOTIF_TYPES.MANUAL });
      onDone?.();
    } catch {
  ...
  return (
    <div className="space-y-2.5 p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 bg-primary/80 hover:bg-primary text-slate-950 font-bold py-2.5 rounded-xl text-xs transition disabled:opacity-50"
      >
        {sending
...
const ViewerDetailPanel = ({ user, isOpen, onClose, onChangeStatus }) => {
  ...
  useEffect(() => {
    ...
    unsubs.push(
      onSnapshot(
        query(collection(db, 'visits'), where('clientEmail', '==', user.email), orderBy('createdAt', 'desc'), limit(10)),
        (snap) => { if (!cancelled) setVisits(snap.docs.map((d) => ({ id: d.id, ...d.data() }))); },
        () => {}
      )
    );
    ...
    unsubs.push(
      onSnapshot(
        query(collection(db, 'contracts'), where('clientEmail', '==', user.email), orderBy('createdAt', 'desc'), limit(10)),
        (snap) => { if (!cancelled) setContracts(snap.docs.map((d) => ({ id: d.id, ...d.data() }))); },
        () => {}
      )
    );
  ...
  function handleScheduleVisit() {
    ...
    onClose();
    navigate(`/agendar-visita?${p.toString()}`);
  }
        ...
        <>
          ...
          >
            ...
            <div className="relative flex-shrink-0">
              ...
              <div className="relative flex items-start gap-4 px-5 pt-6 pb-3">
                <div className="relative flex-shrink-0">
                  {user.photoURL ? (
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {user.status && (
                      <span className={`inline-flex items-center px-2 py-1 rounded-lg text-xs font-medium border ${STATUS_STYLES[user.status] ?? STATUS_STYLES.inactive}`}>
              ...
              <div className="relative px-5 pb-4 flex flex-col gap-2">
                {onChangeStatus && (
                  <button
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 transition"
                  >
                    <FaCalendarCheck className="text-[10px]" /> Agendar visita
                  </button>
                  <button
                    onClick={() => setShowNotif((v) => !v)}
                    className={`flex-1 inline-flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold transition border ${
                      showNotif
                        ? 'bg-purple-500/20 text-purple-300 border-purple-500/30'
                        : 'bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 border-purple-500/20'
                    }`}
                  >
                    <FaBell className="text-[10px]" /> {showNotif ? 'Cerrar' : 'Notificar'}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              {loading ? (
                ...
                <div className="p-5 space-y-5">
                  ...
                  {/* Formulario notificación inline */}
                  <AnimatePresence>
                    {showNotif && (
                      <motion.div
                      ...
                      >
                        <SendNotifForm email={user.email} onDone={() => setShowNotif(false)} />
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { label: 'Favoritos', value: (clientData?.favorites ?? []).length, color: 'text-rose-400',    bg: 'bg-rose-500/10 border-rose-500/15'       },
                      { label: 'Visitas',   value: visits.length,                        color: 'text-blue-400',    bg: 'bg-blue-500/10 border-blue-500/15'       },
                      { label: 'Contratos', value: contracts.length,                     color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/15' },
                  ...
                  <div>
                    ...
                    <div className="bg-white/[0.03] rounded-2xl border border-white/[0.06] px-4 py-1">
                      ...
                      <InfoRow icon={FaMapMarkerAlt}  label="Zona de interés"    value={clientData?.ubicacionInteres} />
                      <InfoRow icon={FaMoneyBillWave} label="Presupuesto"        value={clientData?.presupuesto} />
                      <InfoRow icon={FaBuilding}      label="Tipo de propiedad"  value={clientData?.tipoPropiedad} />
                    </div>
                  </div>
...
// ═════════════════════════════════════════════════════════════════════════════
// PANEL AGENTE (admin / member)
// Sin cambios respecto a la versión original — todas las tabs intactas.
// ═════════════════════════════════════════════════════════════════════════════

const AgentDetailPanel = ({ user, isOpen, onClose, onEdit, onChangeStatus, onResetPassword }) => {
  ...
  useEffect(() => {
    ...
    setVisits([]); setContracts([]); setActivity([]);
    setLoading(true);
    ...
    const loadData = async () => {
      try {
        ...
        if (!cancelled) setStats((prev) => ({ ...prev, properties: propsSnap.size }));
        ...
        console.error('AgentDetailPanel loadData:', err);
      } finally {
        if (!cancelled) setLoading(false);
      }
    };
  ...
  const roleIconCls = ROLE_ICON_CLASSES[user.role] || ROLE_ICON_CLASSES[USER_ROLES.VIEWER];
  const roleGradient = {
            ...
            <div className="relative flex-shrink-0">
              ...
              {(onEdit || onChangeStatus || onResetPassword) && (
                <div className="relative flex gap-2 px-5 pb-5">
                  {onEdit && (
                    <button onClick={() => onEdit(user)} className="flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 bg-primary/10 hover:bg-primary/20 text-primary rounded-xl text-xs font-semibold transition-all border border-primary/20 hover:border-primary/40">
                      <FaEdit className="text-xs" /> Editar perfil
                    </button>
                  )}
                  {onChangeStatus && (
                    <button onClick={() => onChangeStatus(user)} className={`flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold transition-all border ${
                      user.status === 'active'
                        ? 'bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border-amber-500/20 hover:border-amber-500/40'
                        : 'bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border-emerald-500/20 hover:border-emerald-500/40'
                    }`}>
            ...
            <div className="flex border-b border-white/[0.06] flex-shrink-0 bg-black/20">
              {TABS.map((tab) => (
                ...
                >
                  ...
                  {tab.label}
                  {activeTab === tab.id && (
                    <motion.div layoutId="tab-indicator" className="absolute bottom-0 left-2 right-2 h-0.5 bg-primary rounded-t-full" />
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'performance' && (
                  <motion.div key="performance" initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-8 }} className="p-5 space-y-6">
                    {loading ? (
                      ...
                      <>
                        <section>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <KpiCard icon={FaHome}         iconBgClass="bg-amber-500/15"   iconClass="text-amber-400"   value={stats.properties}      label="Propiedades"    sublabel="asignadas" />
                            <KpiCard icon={FaCalendarCheck} iconBgClass="bg-blue-500/15"   iconClass="text-blue-400"   value={stats.visits}          label="Visitas"        sublabel="registradas" />
                            <KpiCard icon={FaFileContract}  iconBgClass="bg-primary/15"    iconClass="text-primary"    value={stats.contracts}       label="Contratos"      sublabel="creados" />
                            <KpiCard icon={FaCheckCircle}   iconBgClass="bg-emerald-500/15" iconClass="text-emerald-400" value={stats.contractsActive} label="Vigentes"       sublabel="activos hoy" />
                          </div>
                        </section>
                        {visits.length > 0 && (
                          <section>
                ...
                {/* ── TAB ACTIVIDAD ── */}
                {activeTab === 'activity' && (
                  <motion.div key="activity" initial={{ opacity:0, x:-8 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-8 }} className="p-5">
                    {loading ? (
                ...
                {/* ── TAB CONTRATOS ── */}
                {activeTab === 'contracts' && (
                  <motion.div key="contracts" initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-8 }} className="p-5 space-y-4">
                    ...
                    {loading ? (
                      ...
                      <EmptyState icon={FaFileContract} title="Sin contratos registrados" subtitle="Los contratos asignados aparecerán aquí" />
                    ) : (
...
// ═════════════════════════════════════════════════════════════════════════════
// EXPORT PRINCIPAL — dispatcher por rol
// ═════════════════════════════════════════════════════════════════════════════

const UserDetailPanel = ({ user, isOpen, onClose, onEdit, onChangeStatus, onResetPassword }) => {
