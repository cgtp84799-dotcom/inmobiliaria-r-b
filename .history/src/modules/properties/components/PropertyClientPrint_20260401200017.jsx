...
export default function PropertyClientPrint({ property, onClose }) {
  ...
  // Escala responsive en vista previa (no afecta al PDF)
  useEffect(() => {
    const el = docRef.current;
    if (!el) return;

    const updateScale = () => {
      const available = window.innerWidth - 32; // padding backdrop
  ...
  const loc = [property.address, property.neighborhood, property.city, property.department]
    .filter(Boolean)
    .join(', ');
  const amenities = [
    ...(property.amenities ?? []),
      ...
      <div className="cf-brand">
        ...
        <div>
          ...
          <div className="cf-brand-sub">
            {subtitle || 'Venta · Arriendo · Finca Raíz · Anserma, Caldas'}
          </div>
        </div>
      </div>
  ...
  const SecHd = ({ children, style }) => (
    <div className="cf-sec-hd" style={style}>
      {children}
    </div>
  );
  ...
  return (
    <div className="cflyer-backdrop" onClick={onClose}>
      <div
        className="cflyer-toolbar no-print"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={handleDownload}
          disabled={loading || !ready}
          className="cflyer-btn-dl"
        >
          {loading
            ? '⏳ Generando PDF...'
            : !ready
        ...
        <button
          onClick={() =>
            printDocument(docRef.current, property.title || 'Ficha Cliente')
          }
          className="cflyer-btn-pr"
        >
          🖨 Imprimir
        </button>
        <button onClick={onClose} className="cflyer-btn-cl">
          ✕ Cerrar
        </button>
      </div>
        ...
        <div ref={docRef} className="cflyer-root">
          ...
          <div className="cflyer-page pdf-page">
            ...
            <footer className="cf-footer">
              <div className="cf-footer-cta">
                ¿Interesado? Comuníquese con nosotros hoy mismo
              </div>
              ...
              <div className="cf-footer-disc">
                ...
                {new Date().getFullYear()} Inmobiliaria Rincón Bedoya y
                Asociados.
              </div>
            </footer>
            <div className="cf-rule" />
          ...
          <div className="cflyer-page pdf-page">
            ...
            <div className="cf-two-col">
              <div>
                ...
                <table className="cf-table">
                  <tbody>
                    ...
                    {isArriendo && property.minimumRentalPeriod && (
                      <Row
                        label="Período mín."
              ...
              <div>
                ...
                {imgs.gallery.length > 0 && (
                  <>
                    <SecHd
                      style={{ marginTop: amenities.length > 0 ? 12 : 0 }}
                    >
                      GALERÍA
                    </SecHd>
                    <div
            ...
            <div className="cf-disclaimer">
              La información presentada tiene carácter informativo y está
              sujeta a verificación. Precios, áreas y condiciones pueden
              variar. Para confirmar disponibilidad comuníquese al{' '}
              <strong>310 596 8202</strong>.
            </div>
