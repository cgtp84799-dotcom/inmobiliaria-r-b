...
export default function PropertyPrintView({ property, onClose }) {
  ...
  const hasPage3 = !!(hasLegal || hasOwner);
  const hasPage4 = !!(amenities.length > 0 || hasNotes || documents.length > 0);
  ...
  const SecHd = ({ children }) => <span className="ppv-sec-hd">{children}</span>;
    ...
    <div className="ppv-backdrop" onClick={onClose}>
      ...
      <div ref={docRef} className="ppv-root" onClick={e => e.stopPropagation()}>
          ...
          {imgs.hero ? (
            ...
            <div className="ppv-hero-empty">
              <div>
                ...
                <h1 style={{ fontFamily: 'Cormorant Garamond,Georgia,serif', fontSize: 24, fontWeight: 700, color: '#fff', margin: '0 0 3px', lineHeight: 1.15 }}>
        ...
        <div className="ppv-page pdf-page">
          ...
          <div className="ppv-page-banner">FICHA TÉCNICA COMPLETA — {property.title?.substring(0, 60)}</div>
                ...
                <>
                  ...
                  <table className="ppv-table">
                    <tbody>
                      ...
                      <Row label={isArriendo ? 'Canon/mes' : 'Precio de venta'} value={fmt(property.price)} />
                      {property.commissionPercentage && <Row label={`Comisión (${property.commissionPercentage}%)`} value={commission ? fmt(commission) : null} />}
                      {property.propertyTax       && <Row label="Predial anual"   value={fmt(property.propertyTax)} />}
                      {property.administrationFee && <Row label="Administración"  value={`${fmt(property.administrationFee)}/mes`} />}
                      {property.rentalDeposit     && <Row label="Depósito"        value={`${property.rentalDeposit} meses`} />}
                      {property.minimumRentalPeriod && <Row label="Período mínimo" value={`${property.minimumRentalPeriod} meses`} />}
                    </tbody>
                  </table>
                </>
              )}
            ...
            <div>
              ...
              <table className="ppv-table">
                <tbody>
                  ...
                  {property.cadastralAppraisal && <Row label="Avalúo catastral" value={fmt(property.cadastralAppraisal)} />}
                </tbody>
              </table>
              ...
              {imgs.gallery.length > 0 && (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                    {imgs.gallery.map((img, i) => (
                      <div key={i} style={{ height: 68, borderRadius: 6, backgroundImage: `url(${img})`, backgroundSize: 'cover', backgroundPosition: 'center', border: '1px solid #e2e8f0' }} />
                    ))}
                  </div>
                </>
              )}
          ...
          <div className="ppv-disclaimer">
            ...
            Para confirmar disponibilidad y condiciones exactas comuníquese al <strong>310 596 8202</strong>.
          </div>
        ...
        {hasPage3 && (
          <div className="ppv-page pdf-page">
            <div className="ppv-rule" />
            <Header subtitle="Información Jurídica y Datos del Propietario" />
            <div className="ppv-page-banner">INFORMACIÓN JURÍDICA · CONFIDENCIAL · USO INTERNO</div>
              ...
              <div>
                {hasLegal && (
                  <>
                    ...
                    {(property.legalStatus || property.cadastralAppraisal || property.horizontalProperty) && (
                      <>
                ...
                {documents.length > 0 && (
                  <>
                    ...
                    <ul style={{ listStyle: 'none', padding: '3px 0 0', margin: 0, fontSize: 8.5, color: '#374151' }}>
                      {documents.map((doc, i) => (
                        <li key={i} style={{ padding: '2.5px 0', borderBottom: '1px solid #f0f0f0' }}>
              ...
              <div>
                {hasOwner && (
                  <>
                    ...
                    <table className="ppv-table">
                      <tbody>
                        <Row label="Nombre completo"     value={property.ownerName}  confidential />
                        <Row label="Teléfono"            value={property.ownerPhone} confidential />
                        <Row label="Correo electrónico"  value={property.ownerEmail} confidential />
                      </tbody>
                    </table>
                  </>
                )}
                ...
                <table className="ppv-table">
                  <tbody>
                    <Row label={isArriendo ? 'Canon/mes' : 'Precio venta'} value={fmt(property.price)} />
                    {property.commissionPercentage && <Row label={`Comisión ${property.commissionPercentage}%`} value={commission ? fmt(commission) : null} />}
                    {property.cadastralAppraisal   && <Row label="Avalúo catastral" value={fmt(property.cadastralAppraisal)} />}
                    {property.propertyTax          && <Row label="Predial anual"    value={fmt(property.propertyTax)} />}
                  </tbody>
        ...
        {/* ══════════════ PÁGINA 4 — AMENIDADES Y NOTAS ══════════════ */}
        {hasPage4 && (
          <div className="ppv-page pdf-page">
            <div className="ppv-rule" />
            <Header subtitle="Amenidades, Características y Observaciones" />
            <div className="ppv-page-banner">AMENIDADES, OBSERVACIONES Y DOCUMENTOS</div>
              ...
              <div>
                ...
                {hasPage3 && amenities.length > 0 && (
                  <>
                    ...
                    <p style={{ fontSize: 8, color: '#64748b', fontStyle: 'italic', margin: '4px 0' }}>
