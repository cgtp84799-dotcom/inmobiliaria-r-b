...
const PropertyDetailPage = () => {
  ...
  const getTransactionType = (type) => {
    ...
    if (["rent", "arriendo", "alquiler", "renta"].includes(lower)) return "Arriendo";
    return type;
  };
  ...
  const baseFeatures = useMemo(() => {
    const items = [];
    if (property?.floors)
      items.push({ type: "base", icon: FaLayerGroup, label: `Piso ${property.floors}` });
    if (property?.stratum)
      items.push({
        type: "base",
        icon: FaCheckCircle,
        label: `Estrato ${property.stratum}`,
      });
    if (property?.yearBuilt)
      items.push({
        type: "base",
        icon: FaCalendarAlt,
        label: `Año ${property.yearBuilt}`,
      });
    return items;
  ...
  const seoDescription = property
    ? `${transType === "Venta" ? "En venta" : "En arriendo"}: ${
        property.title
      } en ${
