...
const CSS_BASE = `
  * { box-sizing: border-box; }
  body { margin: 0; padding: 0; background-color: #f0f4f8; font-family: -apple-system, 'Segoe UI', Helvetica, Arial, sans-serif; }
  ...
  .btn-primary { display: inline-block; text-decoration: none; font-weight: 700; font-size: 15px; padding: 14px 32px; border-radius: 50px; }
...
function infoRow(label, value, accentColor) {
  if (!value) return "";
  // FIX: el bug original era `info-value accent" style="color:${color}` — comilla mal ubicada
  const valueStyle = accentColor
    ? `class="info-value" style="font-weight:700;color:${accentColor};flex:1;font-size:14px;"`
    : `class="info-value"`;
  return `
    <div class="info-row">
