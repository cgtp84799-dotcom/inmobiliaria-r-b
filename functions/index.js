...
async function assertAdminFromRequest(req) {
  ...
  if (!callerEmail) {
    const err = new Error("Token sin email"); err.status = 401; throw err;
  }
...
function getSafeString(value, fallback = "") {
  const r = String(value ?? "").trim(); return r || fallback;
}
...
function extractPropertyImages(data, baseUrl = BASE_URL) {
  ...
  return selected.map((u) => normalizeAbsoluteUrl(u, baseUrl)).filter(Boolean)
...
// ═══════════════════════════════════════════════════════════════════════════
// FUNCIÓN 3: redirectToCustomDomain
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// FUNCIÓN 5: onVisitStatusChanged  ← NUEVA (3D)
// Trigger Firestore: /visits/{visitId} onUpdate
// Envía emails automáticos cuando una visita pasa de pending → approved/rejected
// ═══════════════════════════════════════════════════════════════════════════
...
const fromEmail = () => `"${FROM_NAME}" <${functions.config().gmail?.user ?? process.env.GMAIL_USER}>`;
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#1e293b;border-radius:16px;overflow:hidden;border:1px solid #334155;">
  ...
  <tr><td style="background:#0d1117;padding:16px 32px;border-top:1px solid #334155;text-align:center;">
    <p style="color:#64748b;font-size:12px;margin:0;">${FROM_NAME} · Cra 5 No. 9-28, Anserma, Caldas<br/>
    <a href="${BASE_URL}" style="color:#c9a84c;text-decoration:none;">${BASE_URL}</a></p>
  </td></tr>
</table></td></tr></table></body></html>`;
}

function approvedEmailHtml(d) {
  return htmlWrapper(`
...
function rejectedEmailHtml(d) {
  return htmlWrapper(`
    <h2 style="color:#f87171;font-size:22px;margin:0 0 8px;">Tu solicitud no pudo ser aprobada</h2>
    <p style="color:#94a3b8;font-size:14px;margin:0 0 24px;">Hola <strong style="color:#e2e8f0;">${d.clientName}</strong>, lamentamos informarte que tu solicitud de visita para <strong style="color:#e2e8f0;">${d.propertyName}</strong> no pudo ser confirmada.</p>
  ...
  .onUpdate(async (change) => {
    ...
    if (before.status === "pending" && after.status === "approved") {
      if (after.clientEmail)
        promises.push(transporter.sendMail({ from: fromEmail(), to: after.clientEmail, subject: `✅ Visita aprobada — ${after.propertyName}`, html: approvedEmailHtml(after) }));
      if (after.agentEmail)
        promises.push(transporter.sendMail({ from: fromEmail(), to: after.agentEmail, subject: `🏠 Nueva visita asignada — ${after.propertyName}`, html: agentEmailHtml(after) }));
    }

    // pending → rejected
    if (before.status === "pending" && after.status === "rejected") {
      if (after.clientEmail)
        promises.push(transporter.sendMail({ from: fromEmail(), to: after.clientEmail, subject: `Actualización sobre tu solicitud — ${after.propertyName}`, html: rejectedEmailHtml(after) }));
    }
